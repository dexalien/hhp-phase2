...
const schema = z.object({
  order: z.array(z.object({ id: z.string(), display_order: z.number().int().positive() })),
})
...
export async function PATCH(req: NextRequest) {
  ...
  const updates = parsed.data.order.map(({ id, display_order }) =>
    supabaseServer.from("communities").update({ display_order }).eq("id", id),
  )
